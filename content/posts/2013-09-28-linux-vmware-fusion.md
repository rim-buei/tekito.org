...
結局ぼくの環境では vmhgfs モジュールをビルドして動かすまでに `file.c`, `inode.c` 周り, `vmci.c` 周りを修正する必要がありました。
...
1. `file.c` の修正
   ...
   [vmware-tools-9.2.3-file.patch](vmware-tools-9.2.3-file.patch)
...
2. `inode.c` 周りの修正
   ...
   - [Linux 3.9 の hlist_for_each_entry マクロ](http://lxr.free-electrons.com/source/include/linux/list.h?v=3.9#L674)
   - [Linux 3.7 の vmtruncate() 関数](http://lxr.free-electrons.com/source/mm/truncate.c?v=3.7#L579)

   [vmware-tools-9.2.3-inode.patch](vmware-tools-9.2.3-inode.patch)
...
3. `vmci.c` 周りを修正する
   ...
   [vmware-tools-9.2.3-vmci.patch](vmware-tools-9.2.3-vmci.patch)
